...
def make_network(depths, tile_id, activation=None, bias=True):
    ...
    output_rotation = tf.layers.dense(input_layer, 4, activation=activation, use_bias=False)
    output_column = tf.layers.dense(input_layer, WIDTH, activation=activation, use_bias=False)

    output_column = tf.nn.softmax(output_column)
...
class depths_network:
    ...
    def __init__(self):
        ...
        self.output_rotation, self.output_column = make_network(self.depths, self.tile_ids, activation=None)
